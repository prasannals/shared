...
float x0, y0, x1, y1, x2, y2, x3, y3, maxY, minY;
...
void findX(float y, float *x0, float *x1){
  bool x0done = false;
  ...
  if( (temp = xOf(y, leftt)) != NAN){
    v.push_back(temp);
  }

  if( (temp = xOf(y, rightt)) != NAN){
    v.push_back(temp);
  }
  if( (temp = xOf(y, top)) != NAN){
    v.push_back(temp);
  }

  if( (temp = xOf(y, bottom)) != NAN){
    v.push_back(temp);
  }
